...
If the stack is *empty*, this means that *every opening parenthesis* found a matching *closing parenthesis* in the correct order. Therefore, I return `true` as the input is valid. If the stack is **not** *empty*, then there was some *parenthesis* left unaccounted for, so I return `false`.

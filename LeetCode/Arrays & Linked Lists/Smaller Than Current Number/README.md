...
In the standard **Counting Sort** algorithm: 
 ...
 2. We modify $C$ by adding elements such that $C[i]$ tells us the exact *ending position* of element $i$ by doing $C[i] \mathrel{+}= C[i-1]$. This tracks how many elements are less than or equal to $i$.
...
We can adapt the standard **Counting Sort** logic by making two small adjustments:
 ...
 - We keep the output vector `result` in its original layout. We loop through `nums` *left-to-right* from $j=0 \dots \text{nums.size()}- 1$, using the value of $\text{count[nums[j]]}$ as a **direct lookup table** to populate `result[j]`.
...
vector<int> smallerNumbersThanCurrent(vector<int>& nums) {
    ...
    for (int i = 0; i <= k; i++) {
        ...
        // get number of occurrences of current element
        int current = count[i];

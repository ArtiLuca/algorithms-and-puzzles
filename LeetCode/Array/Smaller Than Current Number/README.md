...
We can adapt the standard **Counting Sort** logic by making two small adjustments:
 ...
 - We keep the output vector `result` in its original layout. We loop through `nums` *left-to-right*, using the value of $\text{count[nums[j]]}$ as a **direct lookup table** to populate `result[j]`.

...
We want to obtain *height-balanced binary search tree* from a sorted array of integers sorted in a **strictly increasing order**, which means there are no *duplicates*. It also means that to obtain the *most height-balanced* BST possible, we must use the middle element as the root.

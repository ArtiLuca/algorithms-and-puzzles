...
We can then perform validation for each password in linear time by either simply performing a linear seach and counting the number of occurrence of the password's corresponding policy character. Or also use the inbuilt method `count(str.begin(), std.end(), c)` which also results in linear time.
...
Keeping into account the positions indicate *1-indexed* positions, we can implement a new boolean validation helper
for the struct `Password`.

In particular, the new validation helper must check that **exactly one** position between the two given by the policy 
contains the needed character, so its as if we were evaluating an exclusive OR (XOR) condition. That is, for any given password `str` having policy character `c` and policy positions `i` and `j`, we must check that the following condition evaluates to true:

$$
psw[i-1] == 'c' \text{ XOR } psw[j-1] == 'c'
$$

We can then use this new validation helper across all passwords to count the total number of valid passwords in accordance with the updated corporate policies.
...
#### Complexity
Assuming the database contains $n$ passwords and that each password has at the most $m$ characters.
The validation of a single password for Part 2 has a constant cost $\Theta(1)$ since  performing the necessary checks remains constant in time and accessing elements of a vector is constant $\Theta(1)$.  

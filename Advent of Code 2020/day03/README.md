...
#### Complexity
The cost of a single call to the helper `countTrees` remains linear with the total number of rows $n$ in the grid, so a single call has a time complexity of $\mathcal{O}(n)$. The time complexity for the first four slopes is $\mathcal{O}(n)$, while the cost for the fifth slope is $\mathcal{O}(\frac{n}{2})$. The total **time complexity** for all five calls can be simplified to $\mathcal{O}(n)$.

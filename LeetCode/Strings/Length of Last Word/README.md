...
## Solution
We are told that $1 \le \text{s.length} \le 10^{4}$, that  `s` consists of only English letters and spaces `' '`, and that there will be at least one word in `s`.
 ...
 * Assuming the input string `s` has $n$ characters. Processing the entire string takes proportional time to its length. Therefore, the total **time complexity** is $\mathcal{O}(n)$.
